...
public class GradientPicker : MonoBehaviour
{
    ...
    public bool Create(Gradient original, string message, GradientEvent onGradientChanged, GradientEvent onGradientSelected)
    {
        if (instance is null)
        {
    ...
    private void ChangeSelectedColorKey(int value)
    {
        ...
        if (selectedColorKey != value && !_colorPicker.done)
        {
            _colorPicker.Done();
        }
    ...
    {
        _colorPicker.Create(colorKeys[selectedColorKey].color, "Gradient Color Key", (c) => UpdateColor(selectedColorKey, c), null);
    }
